...
Before enabling Testnet scheduling, verify the bridge against its provider documentation and test its identity/read methods on the intended chain. Typed signing and the bounded transaction flow should be exercised only within the user's authorized test. Provider-neutral behavior is covered with mocks in this repository; named framework/wallet integrations have not been connected or tested by this draft.
