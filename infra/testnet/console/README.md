...
```json
{
  ...
  "treasury": "0x6fD155b9D52F80E8A73a8A2537268602978486e2",
...
Review the treasury address shown before deployment. Each 110 tRF Generations entry retains 100 for the prize pool and forwards 10 to this immutable address. Genesis remains free. The console verifies these values and the 1,024,000,000-token reward cap after deployment and includes them in the exported manifest. `prepare:operator OWNER [TREASURY]` defaults the treasury to the owner for testnet.
...
Download the final manifest and compiler input after all four deployments are verified. Both contain public deployment information only. Changing compiled artifacts or the treasury blocks reuse of existing deployment progress until it is reconciled. A completely untouched page with no deployment records can safely refresh to the new package. The tool deliberately offers no automatic progress-reset button for actual deployments.
