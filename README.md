...
[Genesis in Motion](https://rarerush.app/genesis-lab/) is a wallet-free animation showcase for players and judges. Compare the original public Genesis #1 portrait with its runner, roll a new body, try idle/run/jump/slide, and inspect all eight running frames. It shares the game's sprite renderer and curated pool of 36 compatible bodies. Genesis selects one body per run without back-to-back repeats; slide squeezes the whole character to fit under obstacles. Bodies remain cosmetic, with unchanged collision and reward rules. The showcase earns no rewards and starts paused when reduced motion is preferred.
