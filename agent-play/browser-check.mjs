...
try {
  ...
  assert.equal(await page.evaluate(() => window.__copiedAgentText), 'node agent-play/cli.mjs run --job agent-play/examples/preview-job.json');
  ...
  assert.equal(await page.locator('.library, .rush-leaderboard').count(), 0, 'Agent Play does not render the old inline leaderboard');
  await page.getByRole('tab', { name: 'AGENTIC', exact: true }).click();
  await selectedTab(page, 'AGENTIC');
  await page.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'LEADERBOARD', exact: true }).click();
  ...
  assert.equal(await page.locator('.leaderboard-row').count(), 1, 'The saved best run survives reload');
  ...
  assert.deepEqual(await leaderboardRow.evaluate(row => [...row.children].slice(0, 3).map(child => child.className)),
    ['leaderboard-rank', 'leaderboard-thumbnail', 'leaderboard-friend'], 'Thumbnail appears between the rank and Friend');
  ...
  await noOverflow(page, 'Saved Leaderboard');
  await page.screenshot({ path: resolve(artifacts, 'saved-leaderboard.png'), fullPage: true });
  check('Watching a replay reproduces final metrics; the saved best run survives reload on the dedicated Leaderboard');
  const leaderboardWatch = leaderboardRow.locator('.leaderboard-watch');
  await leaderboardWatch.click();
  const savedReplay = page.getByRole('dialog', { name: 'Watch saved run', exact: true });
  await savedReplay.locator('.agent-stage-world').waitFor();
  assert.equal(await savedReplay.getByRole('alert').count(), 0);
  await savedReplay.getByRole('button', { name: 'Play replay', exact: true }).click();
  await page.waitForFunction(() => document.querySelector('.replay-modal progress')?.value > 0);
  assert.ok(await savedReplay.locator('progress').evaluate(element => element.value) > 0, 'The saved replay plays inside the popup');
  assert.equal(await page.locator('.watch-section').getAttribute('data-screen'), 'ready', 'Watching a leaderboard replay does not replace the Autopilot session');
  await page.keyboard.press('Escape');
  assert.equal(await leaderboardWatch.evaluate(element => element === document.activeElement), true);
  assert.equal((await (await page.request.get(`${origin}/api/runs`)).json()).runs.length, 1, 'The popup never creates another saved score');
  await page.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'AGENT PLAY', exact: true }).click();
  await selectedTab(page, 'AGENTIC');
  assert.equal(new URL(page.url()).hash, '#agent-play');
  assert.equal(await page.locator('.rush-leaderboard').count(), 0);
  check('Leaderboard WATCH opens a read-only replay popup, restores focus on close, and preserves the selected Agentic tab');
