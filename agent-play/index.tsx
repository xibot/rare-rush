...
import { ArcadeFriendPicker } from './ArcadeFriendPicker.tsx';
...
function App() {
  ...
  const friendSelections=useRef<Record<string,string>>({});
  ...
  function invalidateArcade() {
    setArcade(null);setArcadeArt(null);
    for(const key of Object.keys(friendSelections.current))if(key.startsWith('arcade:'))delete friendSelections.current[key];
    if(sourceRef.current==='arcade')setTokenId('');
  }
  ...
  function chooseSource(next:Source) {
    if(frozen||next===source)return;friendSelections.current[`${source}:${collection}`]=tokenId;setTokenId(friendSelections.current[`${next}:${collection}`]??(next==='arcade'?'':'1'));adapter.current?.cancelFriendCheck();friendCheckAttempt.current='';setSource(next);setError('');setNotice('');setScreen('ready');setRecord(null);setCurrent(null);identity.current=null;setArcadeArt(null);
  ...
  function download(){
    const payload={...identity.current,agent:'Rare Rush autopilot',replay:exportAgentReplay(live.current),...(artRef.current?{art:artRef.current}:{})};
    ...
    <main>
      ...
      <section ref={setupAnchor} className="setup" aria-label="Agent run setup">
        ...
        <div className="setup-grid"><div>
          ...
          {source==='arcade'&&tokenId&&<p className="lime small arcade-selected-friend">SELECTED · {collection===1?'GENESIS':'GENERATIONS'} #{tokenId}</p>}
          {source==='arcade'?<details className="arcade-manual-id"><summary>Know your Friend’s ID? Enter it manually</summary><label className="id-label" htmlFor="friend-id">MAINNET NFT ID<input id="friend-id" value={tokenId} onChange={e=>{adapter.current?.cancelFriendCheck();setTokenId(e.target.value);setArcadeArt(null);setError('');setNotice('');}} placeholder="Friend ID" inputMode="numeric" autoComplete="off" disabled={frozen} maxLength={77}/></label><p className="muted small">Use the ID from your mainnet NFT. Testnet IDs are separate.</p></details>:<label className="id-label" htmlFor="friend-id">{source==='local'?'PREVIEW FRIEND ID':'YOUR NFT ID'}<input id="friend-id" value={tokenId} onChange={e=>{adapter.current?.cancelFriendCheck();setTokenId(e.target.value);setArcadeArt(null);setError('');setNotice('');}} inputMode="numeric" autoComplete="off" disabled={frozen} maxLength={77}/></label>}
          ...
          <button className="primary start" disabled={frozen||!validId(tokenId)||(source==='arcade'&&(!arcade?.account||arcade.chainId!==4663))||(source==='testnet'&&(!tnReady||activeSaved))} onClick={()=>void act(source==='testnet'?(needApproval?'Approve entry in your wallet…':'Confirm the run in your wallet…'):'Preparing the agent…',begin)}>{busy|| (source==='testnet' ? needApproval?'APPROVE 110 tRF':'START AGENT RUN':'WATCH AGENT PLAY')}<span>↗</span></button>
