...
const errors = [];
async function setup({ state = null, mobile = false, chain = '0xb626', runtimeMismatch = false, serverReady = true } = {}) {
    ...
    await page.addInitScript(({ state, key, account, chain }) => {
        if (state && !sessionStorage.getItem('play-test-initialized')) {
            localStorage.setItem(key, JSON.stringify(state));
            sessionStorage.setItem('play-test-initialized', '1');
        }
        window.mockWallet = { account, chain, writes: [], requests: [], listeners: {} };
    ...
    }, { state, key: stateKey(account), account, chain });
    await page.route('**/api/status', r => r.fulfill({ json: { ready: serverReady, chainId: 46630, game: PLAY_CONTRACTS.game } }));
    await page.route('**/api/verify-run', r => { unexpected.push('Unexpected verification request'); return r.fulfill({ status: 503, json: { error: 'Verification fixture unavailable' } }); });
    ...
    await page.route(`${RPC_URL}/**`, r => { const data = r.request().postDataJSON(); return r.fulfill({ json: Array.isArray(data) ? data.map(respond) : respond(data) }); });
...
async function connect(page) { await page.getByRole('button', { name: 'CONNECT WALLET' }).click(); await page.getByRole('button', { name: 'REFRESH' }).waitFor(); }
