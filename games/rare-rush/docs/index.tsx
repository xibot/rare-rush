...
import { createEconomy, formatToken, HALVING_INTERVAL, rewardAt, TOKEN_CAP } from '../economy';
...
function RewardLab({ difficulty, onDifficulty }: { difficulty: Difficulty; onDifficulty: (mode: Difficulty) => void }) {
  ...
  const reward = (bonus: 1 | 10) => {
    const amount = rewardAt(pickups, difficulty, bonus) * (genesis ? 100n : 1n);
    const remaining = TOKEN_CAP > scenario.issued ? TOKEN_CAP - scenario.issued : 0n;
    return formatToken(amount < remaining ? amount : remaining);
  };
  return <div className="reward-lab">
    ...
    <div className="holder-preview"><label htmlFor="reward-holder">CHOOSE YOUR FRIEND</label><select id="reward-holder" value={genesis ? 'genesis' : 'generations'} onChange={event => setGenesis(event.target.value === 'genesis')}><option value="generations">Generations · 1× (demo)</option><option value="genesis">Genesis · 100× (planned)</option></select><p role="status">{genesis ? 'GENESIS CONCEPT: 100× tokens per coin. This boost is not active in the arcade yet.' : 'GENERATIONS DEMO: the rewards available in the current arcade simulation.'}</p></div>
    ...
    <div className="reward-results" aria-live="polite"><div className="reward-result"><Coin/><div><span>ORDINARY COIN</span><strong data-reward="ordinary">{reward(1)}</strong><span>{genesis ? 'planned' : 'demo'} tokens / coin</span></div></div><div className="reward-result"><Coin size={82}/><div><span>FLYING BONUS · 10×</span><strong data-reward="bonus">{reward(10)}</strong><span>{genesis ? 'planned' : 'demo'} tokens / coin</span></div></div></div>
    <p className="lab-caption">Generations uses the current demo rules. Genesis previews the proposed 100× boost. Both examples imagine earlier ordinary coins collected on Normal by Generations. This is not a live player counter, wallet balance, or token-price chart.</p>
  </div>;
}
...
function Docs() {
  ...
  return <div className="rush-docs" id="top">
    ...
    <header className="docs-header"><a className="docs-logo" href="/" aria-label="Rare Rush by Xibot home"><BrandMark/></a><nav aria-label="Main navigation"><a href="/">HOME</a><a className="docs-cta" href="/play/">ENTER ARCADE ↗</a></nav></header>
    <main>
      ...
      <div className="docs-layout">
        ...
        <div className="docs-content">
          ...
          <section className="docs-section" id="rewards" aria-labelledby="rewards-title"><div className="section-kicker">04 / INSIDE THE COINS <span className="status-tag">DEMO + PLANNED BOOST</span></div><div className="section-heading"><h2 id="rewards-title">Early coins. Bigger rewards.</h2><p>The idea is simple: start generous, then reduce rewards as more coins are collected. Try the demo curve, or preview a Genesis Friend’s proposed 100× boost.</p></div>
            <RewardLab difficulty={difficulty} onDifficulty={setDifficulty}/>
            <div className="quickstart-grid"><article className="quickstart-card"><b>÷2</b><h3>A smaller slice.</h3><p>The base reward halves every {HALVING_INTERVAL.toLocaleString('en-US')} pickups: 10, then 5, then 2.5… Activity means collected coins, not new wallets or empty runs.</p></article><article className="quickstart-card"><b>×</b><h3>Your mode matters.</h3><p>Apply the difficulty multiplier, then 10× for a flying bonus. All modes share the same demo counter and supply cap.</p></article><article className="quickstart-card"><b>MAX</b><h3>One shared limit.</h3><p>The demo caps total issuance at {formatToken(TOKEN_CAP)} tokens. When the cap is reached, coin rewards become zero. The score chase continues.</p></article></div>
            <p className="docs-note">These are adjustable playtest values. Today’s counter and balances live only in your session. Higher difficulties and bonus coins can use up the supply faster; a reward curve alone does not guarantee economic balance.</p>
          </section>
          ...
          <section className="docs-section" id="next" aria-labelledby="next-title"><div className="section-kicker">05 / THE NEXT LEVEL <span className="status-tag">PLANNED</span></div><div className="section-heading"><h2 id="next-title">Play to mint. Keep it rare.</h2><p>The vision: your verified coin collections earn newly minted <strong>RARERUSH</strong>. Think of it as mining through gameplay: your skill does the collecting, with no mining hardware needed.</p></div>
            ...
            <div className="holder-grid"><article className="holder-card"><span className="status-tag">PROPOSED · 100×</span><h3>Genesis brings the big rush.</h3><p>Free entry and <strong>100× tokens per coin</strong> for the original Friends. The boost stacks with your difficulty and flying bonus coins, within the shared supply cap.</p><p>At the launch rate on Normal: 1,000 per ordinary coin, or 10,000 per flying bonus. Genesis access and rewards still need to be added; Genesis-only wallets cannot enter this build yet.</p></article><article className="holder-card"><span className="status-tag">PROPOSED · 1×</span><h3>Generations fuels the pool.</h3><p>Standard coin rewards, plus your difficulty and bonus multipliers. A small RAREFRIENDS entry fee would go to a prize pool for later rewards. The final fee and prize rules are still to be decided.</p></article></div>
            ...
            <p className="docs-note">Try the idea today: each run uses 1 demo RF from 100 starting credits, and 100% goes to the demo pool. No wallet funds are charged. Prize payouts are not implemented.</p>
            <div className="roadmap-strip"><b>NEXT UP</b><p>Verified run rewards · final token rules · Genesis entry and 100× rewards · funded liquidity · prize distribution. These are design goals, with no launch date set.</p></div>
          </section>
          ...
          <section className="docs-section" id="faq" aria-labelledby="faq-title"><div className="section-kicker">06 / GOOD TO KNOW</div><div className="section-heading"><h2 id="faq-title">A few rare answers.</h2></div><div className="faq-list">
            ...
            <details><summary>Can I change difficulty after a run?</summary><p>Yes. Choose CHANGE DIFFICULTY on the results screen to go back to the selector. RUN IT BACK starts again in the same mode. A demo entry fee applies only when you start a run.</p></details>
            <details><summary>Can I play on my phone?</summary><p>Yes. Use your wallet’s in-app browser with an eligible hardwired Generations NFT to connect. Once in the arcade, use JUMP, SLIDE, SLOW, and FAST. Anyone can watch the home-page preview without connecting.</p></details>
      ...
      <section className="docs-endcap"><span className="section-kicker">YOU KNOW THE WORLD. NOW GO RUN IT.</span><h2>Less reading.<br/>More rushing.</h2><p>Your next best run is one jump away.</p><a className="docs-cta" href="/play/">LET’S RUSH ↗</a></section>
    </main>
