...
export function EntityArt({ entity: e, elapsed, reduced, mirrored = false }: { entity: RunState['entities'][number]; elapsed: number; reduced: boolean; mirrored?: boolean }) {
  ...
  if (e.kind === 'bonus') return <g data-bonus-coin={e.id} data-bonus-x={e.x} data-bonus-y={e.y} transform={`translate(${e.x} ${e.y})`}>
    ...
    <text transform={mirrored ? "translate(50 0) scale(-1 1)" : undefined} x="25" y="-12" textAnchor="middle" fill="#000" fontFamily="var(--rush-font-mono)" fontSize="13" fontWeight="700">10×</text>
  </g>;
  ...
  if (e.kind === 'shield' || e.kind === 'magnet') return <g transform={`translate(${e.x} ${e.y})`}>
    ...
    <text transform={mirrored ? `translate(${e.w} 0) scale(-1 1)` : undefined} x={e.w/2} y={e.h*.74} textAnchor="middle" fill="#000000" fontSize={e.h*.72} fontWeight="bold">{e.kind === 'shield' ? 'S' : 'M'}</text>
  </g>;
