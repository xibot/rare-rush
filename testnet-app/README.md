...
Set `RUSH_RPC_URL` as another server-only **Secret** in that project to use a managed Robinhood Testnet HTTPS endpoint (including its API key). Select Production and Preview, then redeploy to apply it. Both verifier readiness and run verification use this endpoint; browser reads and wallet transactions keep their existing connections. The URL never belongs in public deployment JSON, client code or a `VITE_` variable. If the setting is absent, the verifier uses the public Robinhood Testnet RPC. An invalid configured URL disables verification instead of silently falling back; the existing chain, contract and engine checks still reject the wrong network. Upstream URLs and errors are never returned to clients.
