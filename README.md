...
**This is a simulated economy prototype.** Entry fees, the reward token and prize pool have no monetary value and do not make transactions. The game implements a working local preview of the intended diminishing reward model. Genesis free access, real token minting and the token pair are future integrations.
...
Run `npm run typecheck:rush`, `npm run test:rush`, `npm run check`, `npm run build`, `npm run test:browser`, `npm run test:landing`, and `npm run test:bonus` to validate Rare Rush. The unmodified FriendSDK v0.1.2 package is included for reproducibility. `dist/` is the static hosting output, with the landing page at its root and the SDK game inside `play/`.

The vibeathon submission remains a draft until the builder is happy with the playtest.

## Deployment

Import this repository into Vercel as the `rarerush` project. The committed `vercel.json` uses Node.js 22, installs locked dependencies with `npm ci`, runs `npm run build`, and publishes only `dist/`. It preserves `/play/` and the SDK sandbox documents without a catch-all rewrite. No environment variables or wallet secrets are required. The intended production domain is `rarerush.vercel.app`; deployment remains separate from vibeathon submission.
