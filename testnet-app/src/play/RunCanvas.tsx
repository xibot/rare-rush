...
function RunSession(props: RunCanvasProps) {
  ...
  const world = <svg ref={stage} className="world-svg rush-run-world" viewBox={`0 0 ${viewWidth} 500`} preserveAspectRatio="xMidYMid meet" tabIndex={0} role="img" aria-label={vertical ? `Runner world. ${run.phase === 'up' ? 'Pulled upward' : 'Free falling'} automatically. Hold left or right to steer around obstacles.` : `Runner world. Running ${leftward ? 'left' : 'right'}. Space or up to jump. Down to slide. ${leftward ? 'Left speeds up, right slows down.' : 'Left slows down, right speeds up.'}`} onPointerDown={event => { if (live) { event.preventDefault(); stage.current?.focus({ preventScroll: true }); pressJump(); } }}>
